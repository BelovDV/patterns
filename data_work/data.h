...
namespace Data
{
	const std::string _file_name = "data";

	const int _unit_number_info = 10;
	const int _unit_number_skills = 2;
	const int _class_number_skills = 2;
	...
	namespace Class
	{
		...
		struct Data
		{
			Transform_active start_active[_unit_number_info];
			Transform_passive start_passive[_unit_number_info];
			std::bitset<_class_number_skills> skills;
			int id;
			std::string name;
		};
	}
	...
	namespace Unit
	{
		...
		struct Info
		{
			int parameter[_unit_number_info];
			std::bitset<_unit_number_skills> skills;
		};
...
{
	for (int i = 0; i != Data::_unit_number_info; ++i)
		in >> info.parameter[i];
	in >> info.skills;
	return in;
}
